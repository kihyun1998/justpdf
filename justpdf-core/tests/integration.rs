...
fn test_open_minimal_pdf() {
    ...
    assert_eq!(catalog.as_dict().unwrap().get_name(b"Type"), Some(b"Catalog".as_slice()));
}
...
fn test_two_pages() {
    ...
    let pages_ref = catalog.as_dict().unwrap().get_ref(b"Pages").unwrap().clone();
...
fn test_with_text_stream() {
    ...
    let iref = IndirectRef { obj_num: 4, gen_num: 0 };
...
fn test_incremental_update() {
    ...
    assert_eq!(info_dict.get(b"Title"), Some(&PdfObject::String(b"Test Document".to_vec())));
    assert_eq!(info_dict.get(b"Author"), Some(&PdfObject::String(b"justpdf".to_vec())));
...
fn test_object_not_found() {
    let mut doc = PdfDocument::open(&fixture("minimal.pdf")).unwrap();
    let result = doc.resolve(&IndirectRef { obj_num: 999, gen_num: 0 });
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), JustPdfError::ObjectNotFound { .. }));
}
...
fn test_corrupted_xref_bad_offset() {
    ...
    let result = doc.resolve(&IndirectRef { obj_num: 1, gen_num: 0 });
    assert!(result.is_err());
}
