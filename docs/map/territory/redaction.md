...
- 이미지: 직전 `cm` 하나로 계산한 상자가 영역과 겹치는 `Do`(이미지·Form 구분 없음)를 지운다. q/Q 스택도 행렬 누적도 없다.
- **벡터 경로는 지우지 않는다**. Form XObject 내부는 보지 않고 이미지 픽셀은 바꾸지 않는다.
- 걸러낸 연산자는 `format_operand`로 다시 쓴다 — [객체 구문 왕복](../invariant/object-syntax-roundtrip.md).
...
- Tracked: #29 (손으로 쓰는 구문 이스케이프), #39 (리댁션 잔존)
