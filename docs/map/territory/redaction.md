...
- 걸러낸 연산자는 `write_content`로 다시 쓴다 — 영역 밖의 연산자(괄호 짝 안 맞는 문자열, 인라인 이미지 포함)는 되읽으면 같다(`test_redaction_keeps_content_outside_the_area_unchanged`, #29). 덮개 사각형과 색은 `Number`로 쓴다(#90) — [객체 구문 왕복](../invariant/object-syntax-roundtrip.md)의 생성기 실수.
