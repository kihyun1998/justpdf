...
- `set_font`·`draw_image`·`draw_inline_image`는 이름을 `name_syntax`로 쓴다(#29). 좌표·색 실수는 `write!("{}", f64)` 그대로다 — [객체 구문 왕복](../invariant/object-syntax-roundtrip.md)의 생성기 실수 판단.
...
- Tracked: #33 (텍스트 문자열 인코딩), #34 (비 ASCII 콘텐츠 텍스트), #43 (인라인 이미지 cm 누락), #77 (비암호화 `/ID` 부재), #88 (인라인 이미지 데이터의 `EI` 경계), #90 (생성기 실수)
