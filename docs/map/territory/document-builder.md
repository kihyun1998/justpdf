...
## Code
- `justpdf-core/src/writer/document.rs` — `DocumentBuilder`, `add_standard_font`, `embed_truetype_font`, `set_title`, `set_xmp_metadata`, `set_encryption`, `build`, `save`, `embed_jpeg`, `embed_png`, `generate_tounicode_cmap`
...
- Tracked: #29 (손으로 쓰는 구문 이스케이프), #33 (텍스트 문자열 인코딩), #34 (비 ASCII 콘텐츠 텍스트), #43 (인라인 이미지 cm 누락)
