...
## Design model
- **규칙은 한 곳**: `write_name`·`write_string`·`write_real`(`fmt::Write`에 쓴다)이 규칙이고, 생성된 콘텐츠의 실수는 `Number`(정수값은 정수 — [객체 구문 왕복](../invariant/object-syntax-roundtrip.md)의 생성기 실수, #90)로 쓴다. Display·`serialize_dict`·콘텐츠 연산자 쓰기(`ContentOp::write_to`)가 이것을 부른다. 바이트 버퍼에는 `ByteSink`로, 조각 문자열이 필요하면 `name_syntax`·`string_syntax`·`real_syntax`로 쓴다(#29 전에는 이름 규칙이 세 벌 — Display `Name`, Display `Dict` 키, `write_escaped_name` — 이었다).
