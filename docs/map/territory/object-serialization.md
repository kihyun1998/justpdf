...
## Design model
- **규칙은 한 곳**: `write_name`·`write_string`·`write_real`(`fmt::Write`에 쓴다)이 규칙이고, Display·`serialize_dict`·콘텐츠 연산자 쓰기(`ContentOp::write_to`)가 이것을 부른다. 바이트 버퍼에는 `ByteSink`로, 조각 문자열이 필요하면 `name_syntax`·`string_syntax`·`real_syntax`로 쓴다(#29 전에는 이름 규칙이 세 벌 — Display `Name`, Display `Dict` 키, `write_escaped_name` — 이었다).
- **이름**: 구분자·공백·`#`·`<= 0x20`·`>= 0x7F` 바이트를 `#XX`로 이스케이프한다. `serialize_dict`는 Display `Dict`의 두 번째 사본이다(스트림 값을 `serialize_object`로 쓰는 것만 다르다).
...
## Code
- `justpdf-core/src/object/types.rs` — `PdfObject`, `write_name`, `write_string`, `write_real`, `name_syntax`, `string_syntax`, `real_syntax`, `ByteSink`, `test_string_display_high_byte_uses_hex`, `test_name_display_with_space`
