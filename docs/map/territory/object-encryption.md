...
- [문서 빌더](document-builder.md), [문서 수정기](document-modifier.md) — 파일 ID를 정하는 두 사이트. 둘 다 [파일 직렬화](file-serialization.md)의 `serialize_writer_encrypted`로 쓴다.
...
## Known holes / open
- `generate_file_id`는 공개인 채로 쓰이지 않는다. 원본 `/ID` 첫 원소를 읽는 공개 접근자가 없다(`extract_file_id`는 `pub(crate)`). Tracked: #76.
- `EncryptionConfig::build`가 돌려주는 `/ID` 배열(`make_id_array`, 두 원소 같음)은 core의 두 writer가 쓰지 않는다 — `serialize_writer_encrypted`가 영구·변하는 식별자로 배열을 따로 만든다.
