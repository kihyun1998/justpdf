...
## Known holes / open
- CLI `encrypt`가 다시 쓴 파일의 `/ID` 둘째 원소가 첫째와 같다(`make_id_array`). ISO 32000-1 §14.4로는 다른 버전이므로 달라야 한다. Tracked: #75.
- `generate_file_id`는 공개인 채로 쓰이지 않고, 원본 `/ID` 첫 원소를 읽는 `extract_file_id`는 비공개라 `cmd_encrypt`가 같은 규칙을 따로 구현한다. Tracked: #76.
